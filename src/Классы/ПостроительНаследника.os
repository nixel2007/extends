...
// Константы для имен полей конструктора
Перем ИМЕНА_ПОЛЕЙ_КОНСТРУКТОРА;
// Количество параметров метода ОбработкаПолученияПредставления
...
Функция МетодУжеОпределенВНаследнике(МетодРодителя)
	...
	ЭтоФункцияВНаследнике = МетодРодителя.ЭтоФункция 
		И _РефлекторОбъекта.ЕстьФункция(МетодИмя, КоличествоПараметров);
	ЭтоПроцедураВНаследнике = НЕ МетодРодителя.ЭтоФункция 
		И _РефлекторОбъекта.ЕстьПроцедура(МетодИмя, КоличествоПараметров);
...
Процедура СоздатьМетодПредставления(Декоратор, ИмяТипаРодителя)
	...
	ЕстьОбработкаПредставления = _РефлекторОбъекта.ЕстьПроцедура("ОбработкаПолученияПредставления", 
		КОЛИЧЕСТВО_ПАРАМЕТРОВ_ПРЕДСТАВЛЕНИЯ)
		ИЛИ _РефлекторОбъекта.ЕстьПроцедура("OnGettingPresentation", 
		КОЛИЧЕСТВО_ПАРАМЕТРОВ_ПРЕДСТАВЛЕНИЯ);
