...
Функция СоздатьДекоратор() Экспорт
	...
	Если _КонструкторНаследника = Неопределено Тогда
		...
		Если КонструкторНаследника = Неопределено Тогда
			КонструкторНаследника = МетодыНаследника.Найти("OnObjectCreate", "Имя");
		КонецЕсли;
	...
	Для Каждого МетодРодителя Из МетодыРодителя Цикл
		...
		Если (МетодРодителя.ЭтоФункция И _РефлекторОбъекта.ЕстьФункция(МетодРодителя.Имя, МетодРодителя.КоличествоПараметров))
			ИЛИ (НЕ МетодРодителя.ЭтоФункция И _РефлекторОбъекта.ЕстьПроцедура(МетодРодителя.Имя, МетодРодителя.КоличествоПараметров)) Тогда
	...
	Если КонструкторРодителя = Неопределено Тогда
		КонструкторРодителя = МетодыРодителя.Найти("OnObjectCreate", "Имя");
	КонецЕсли;
	...
	ЕстьОбработкаПредставления = _РефлекторОбъекта.ЕстьПроцедура("ОбработкаПолученияПредставления", 2)
		ИЛИ _РефлекторОбъекта.ЕстьПроцедура("OnGettingPresentation", 2);
